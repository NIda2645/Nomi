...
- **门的身份是「文件 + 符号」，门表里不存行号**（2026-09-18 改）。此前钉着 `path:line`，于是每次有人在门的上面
  插几行，门表就报 `door does not resolve`——长出一族「门表的行号跟上 xxx」的提交（批次 3 一次集成修了 16 处），
  而它们一次都没有发现过真正的门增删。现在同一个文件里调同一个符号三次仍算**一扇**门；
  合同里再写 `line` 直接红（重跑 `door-map.mjs`）。删掉一扇门（文件不再提到那个符号）照样红。
