...
4. **尺寸由内容派生** —— 面板 / 弹层 / 列表的宽高由**里面真正有的东西**决定，不预先占一个固定尺寸；
   **用户只该看需要看的那块，空白不是留白是冗余**（2026-09-14 用户原话：「大片都是空白，理论上不需要，
   ...
   落到具体判据上是三条：
   - 一组可点项**一行放得下就放一行**，放不下按内容换行；**不许每项独占一整行**（那是一列候选列表的摆法，
     只给「长枚举 + 搜索框」那一种用）。
   - **标签短就不撑宽**：不要用等宽/撑满让一个 `720p` 拉成一整条。等宽只留给需要对齐图形槽的语义组（比例）。
   - **浮层不吃固定宽**：只有一组内容时宽度 = `max-content`（可以给一个「最宽不超过」的上限），
     不是预设的面板列宽。多组内容要对齐小标题时才用固定列宽。

   **反例（真实退回）**：单参数直出那一格把 7 个 `1024x1024` 排成 7 行全宽按钮，装在 320px 固定宽的浮层里——
   每行大半空白、整块又高又宽。根因不是样式没调，是**摆法按标签长度分了两种叉**（短→等宽一排、长→一项一行），
   而那第二种叉本身就是冗余。修法是删掉分叉，只留「按内容宽换行」一种
   （`src/workbench/generationCanvas/nodes/parameterOptionPresentation.ts` 的 `parameterOptionLayout`）。
