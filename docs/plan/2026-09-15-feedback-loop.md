...
反馈与用量的**同意语义不同**（④：反馈不受闸）、**投递语义也不同**（用量攒批，反馈要立刻拿到编号）。所以不把反馈塞进 `telemetryOutbox`（那会让 `enabled` 闸长出一个例外分支，闸子一有例外就迟早漏），而是抽一份**通用落盘队列** `electron/telemetry/intakeQueue.ts`，被两个调用方按不同参数用：
