...
状态：🚧 进行中——旅途测试系统与能力面盘点已落地并入库，旅途矩阵作为持续验收门岗运行；本文记录方法与逐维度覆盖，不代表所有旅途已一次性跑通归档。
...
4. 盘点器对生产入口/枚举/预设做静态深比较，防止新增接入面无旅途、或旧面被误删（旅途归属超集校验）。
...
## 2026-09-01 返工对照（接入 #254 事实源）

本轮把这套旅途 harness 接到已落地的 `#254` 事实源，不再自带第二份 manifest：

- **单一事实源**：删除本套自带的 138 行 `manifest.mjs`；journey 事实、维度覆盖面、必测 profile 全部取自 `#254` 的 335 行 `manifest.mjs` + `electron/shared/modelAccessCapabilities.ts`。harness 只保留自己需要、而 manifest 不承载的东西（`harnessContract.mjs`：每条旅途负责的抽屉组件 + 明确排除项）。
- **去重**：`inventory.test.mjs` 只留「生产类型 → 旅途覆盖」这类**独有**反漂移检查（billing/task/provider/auth 从 `modelAccessCapabilities` 常量读，ingestion 从 `AssetIngestion` 判别式读，slots 从 `videoCapabilities/types` 的 owner 读，outputs/modeShapes/entryComponents/providerPresets 从真实生产文件读）；「profile 单一 owner」「协议/恢复/证据宇宙」「脚本存在」三项与 `#254` `manifest.test.mjs` 重叠，已删。
- **J16 移除**：`#254` 事实源没有 `existing-model-preservation`，故删掉 J16 用例与其磁盘深比较 helper（原验收门 #4 已改写为盘点器的超集校验）。
- **抽屉漂移已对账**：main 的 `ModelSettingsHome` 新增了 11 个本地渲染组件（`AdapterTaskWorkspace`/`KnownVendorKeyConnectPage`/`ModelSettings*`/`ConnectionWorkspacePage`/`ModelCapabilityEditor`/`AntigravityConnectionCard`/`IntegrationConfirmationPanel` 等）；已按「已覆盖接入法的页面/任务变体 → 归到对应旅途（J02/J05）」「设置/工作区导航壳 + 独立认证流 → 作用域排除并写明理由」逐一归位，盘点器据此恢复绿。`AvailableGroup`/`ConnectAssistantCard` 已不再渲染，从排除表删除。

### 已知阻塞：roundtrip driver 需按 main 现行模型设置 IA 重配（诚实标注，未伪报）

真跑了 J01/J02（`node run-journeys.mjs J01 J02`，fixture-server 提供假供应商，零真实额度）。结果 **`HARNESS_ERROR`，不是 `PASS`**，证据落在 `.tmp/model-access-journeys/*/J01/report.json`：

- **基建证明可用**：Electron 生产构建正常起、`launchNomiApp` 静态探测 + 重签通过、`fixture-server`/`evidence`/`run-journeys` 编排链工作，driver 能进到「设置 → 模型」区。
- **driver 选择器整体过期**：`#248` 的 `ui-driver.mjs` 写于旧「抽屉分组卡」IA；main 已把模型设置重构成 `ModelSettingsHome` 的「添加服务」网格 + 多屏 connect 流。本轮已按**真机探针**（非源码猜测）修好可确认的两处（去掉不存在的「接入生成模型」展开步、relay 入口改为「自定义 API / 中转站」），但后续每一屏（向导标题「添加一个 AI 模型」、拉取/选择模型、节点执行、结果验真）都需重新对锚——这是**完整重配 driver 导航**，属独立后续单，不在本次「接事实源 + 去重」范围内。`#254` 的 `data-model-access-entry` 锚点已在代码里，但只挂在更深的 connect/detail 屏，driver 到那几屏才用得上。
- **诚实姿态**：`.walk.mjs` 委托到 `run-journeys`，driver 未重配完前会如实 `HARNESS_ERROR`、**不会伪装通过**。合入顺序：必须先 `#254`（事实源）后本单。
