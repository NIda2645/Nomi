...
> 最后核对：2026-08-31 · 核对基线：任务分支 `codex/asset-upload-routing-20260830`
...
| **本地资产上传路由** | 普通 mapping、自定义调用和 Replicate 拆解都在付费供应商请求前经过同一 `AssetIngestion` resolver；按图片/视频/音频能力排序为可选 Nomi R2 relay → 目标/已配置供应商上传 API → 同意后匿名链。支持 KIE、APIMart、fal、Replicate、Runway、RunningHub 的声明式上传协议 | `electron/catalog/assetLocalization.ts:745`、`electron/runtime.ts:263`、`electron/catalog/customCallDispatch.ts:76`、`electron/image/decomposeLayers.ts:62` | ❌「只要有匿名图床就解决了」——匿名 host 仍是最后兜底；R2 未配置时不会伪装成可用，供应商 key/额度/模型输入字段仍需分别验证 |
