...
工作流库条目由可编辑的元数据和不可变的内容快照组成。快照包含选中节点、选区内部边、分组、节点相对位置、插件 envelope、引用的本地素材和当前可复用的生成结果；外部边不复制，避免跨上下文产生隐式副作用。元数据至少包含名称、可选描述、标签、封面引用、来源项目标签和时间戳。
...
当前 `CanvasWorkflowTemplate` 还只有节点和内部边（`src/workbench/generationCanvas/plugins/canvasWorkflowTemplates.ts:3-16`），捕获与实例化均未处理分组；这是实现前必须补齐的合同缺口，而不是由 UI 原型掩盖的已完成能力。

流程库是应用级本地数据，不绑定单个项目。条目索引写入受控的应用设置目录，二进制素材写入由宿主管理的 bundle 目录；renderer 只通过窄类型 bridge 读写，不能获得任意文件系统能力。保存时对本地素材做内容哈希和去重，项目删除后库条目仍可独立复制。

应用时宿主重新分配节点/边/素材 id，重写目标项目内引用，保持可见内容、相对布局和插件状态一致，使用现有 `addNode`/图写入边界或同等事务入口，写入一次 undo barrier 和 persist revision。封面只是检索线索，不参与复制语义：有媒体时默认取保存快照中第一份可见媒体（视频取首帧并标视频），无媒体时使用结构缩略图，不调用 AI 进行选择或改写。
