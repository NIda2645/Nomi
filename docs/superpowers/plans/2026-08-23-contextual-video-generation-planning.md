...
**Completed evidence (2026-08-24):** scoped review PASS; full project gates PASS (699 files / 6174 tests, one skipped); real MCP smoke PASS (45 assertions, 10 steps, six mock-vendor requests, zero provider quota); catalog-backed editable journey PASS (Seedance standard/fast/mini, Veo, Hailuo; aliases, invalid variants, variant parameter limits, same-model edits). A user-authorized real APIMart Seedance smoke was then attempted at 4s/480p/16:9 with audio off: the non-interactive test entry had no client confirmation surface, so the explicit paid-confirmation gate stopped before provider; the externally supplied key separately returned HTTP 401 `invalid API key`; no task or quota was created. This does not make Nomi the only confirmation surface: the intended path is confirmation in the active MCP client when it supports elicitation, with Nomi as fallback. The remaining decision is operational (use a client with interactive confirmation and a valid configured key, or use Nomi fallback/provide a valid replacement key), not a reason to redesign the shared abstraction.
