...
4. 应用级跨项目流程库：框选保存完整包、元数据编辑、混合封面、素材 bundle 持久化和关闭重开恢复。
...
4. 建立应用级 library index + 受控 asset bundle 存储，接入窄类型 bridge；不把二进制素材塞进 Zustand 或 JSON 快照。
...
- 库条目独立于来源项目，来源项目删除后仍可复制；素材按内容哈希去重。
