...
| T-QA-21 | 概念登记表门岗 `check:concept-owners`（发版后第一批）：`docs/engineering/concept-owners.json` 今天只有人读，没有任何机器判据保证「同一个概念只有一个写口」 | todo | 09-22 并行 lane 概念分裂复盘 · 规则进了 R33 | 语义级门岗：把根因合同的门表（`node scripts/door-map.mjs` → `doors`）映射到登记概念，同一概念出现第二个写口即红；棘轮只减不增、pending 条目带收口人。**加规则前先验它会红**（R17）——09-22 那四个概念里至少参数准入与供应商落家会红。与 T-QA-20 ① 的别名族判据（`check:identity-compare`）是同一族，一起排 |
