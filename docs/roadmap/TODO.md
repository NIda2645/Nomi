...
| T-ED-06 | 拆解中断后分镜表节点**永久卡死不报错** | doing `fix/deconstruction-node-terminal-state-20260922` | [付费走查 09-17 §6.5](../audit/2026-09-17-post-804-walkthrough.md) · 09-22 用户拍板为发版前阻断 | 终态保证缺一条：每个节点必须落到成功/失败/可找回三者之一，没有「永远在跑」这一格。**根因不是没人收敛，是收敛完又被盖回去**：2026-09-10 在 `canvasSnapshotNormalizer` 加过一句 `running → idle`，但拆解进度的每一下写都走 `canvas.node.updated` 进了事件日志，hydrate 里 `applyEventTail` 紧跟在收敛之后把 running 原样写回。再叠上「`deconstructToShotTable` 对 running 早退」「footer 重试钮只认 failed/idle」= 点不动的空白格。**已修**：终态判定收成唯一 owner `deconstructionLifecycle.ts`（在飞登记与渲染进程同寿命，进程没了登记自然空），两条读路径共用同一个收敛且**重放之后再问一次**；状态词表加 `interrupted`（中断，可重试）与 `cancelled`（用户取消，不记成失败），i18n 两语；新增「取消拆解」入口。五种中断逐条判过：进程重启 / 切项目 / 用户取消三条本来落非终态或无出口（已修），ffmpeg 子进程死与供应商超时两条本来就落终态（加测钉住不回退）。方案 [`docs/plan/2026-09-22-deconstruction-node-terminal-state.md`](../plan/2026-09-22-deconstruction-node-terminal-state.md) · 合同 `docs/fixes/2026-09-22-deconstruction-node-terminal-state.root-cause.json` · 真机冷重启走查 `tests/ux/deconstruction-interrupted-recovery.walk.mjs`（零成本）。走查顺带抓到一条真问题并同批修掉：中断文案原本被 `i18n.t()` 译好写进**落盘**的 `errorMessage`，切语言不跟随——改成只落机器可读的 status、句子在渲染时按当前语言取。合入 main 后改 done |
