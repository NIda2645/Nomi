...
> **2026-09-17 证据修订**：下文保留当时的推理与决策过程，但不能再把 802 当前失败归因直接写成旧端口。真实 Electron 对照证明 `make_artifact(text)` 首先被媒体专用存储准入拒绝，错误跨 contextBridge 后才被误标成 `surface_port_unavailable`；放行存储后还暴露受控节点尺寸投影导致不可见。长寿命对象持有短命端口的结构债仍成立，需独立证据验收。用户随后批准 A 修复实际失败链及文件 IO 隔离、B 完成项目会话，两部分全部合入；若隔离依赖 B 则 B 必须先完成。当前执行方案见 [PR 802 根因修复与项目会话结构](../../plan/2026-09-17-pr802-root-causes.md)。
