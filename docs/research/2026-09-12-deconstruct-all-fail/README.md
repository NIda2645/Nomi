...
7. ~~**P1 · 切点阈值按 fps 派生**（见上），删掉 `0.01`。~~ **未采纳（2026-09-17）**：碎镜那条已由
   已合入 main 的 #795「秒数吸到 0.1s 格子」解掉（`quantizeShotSeconds` 是镜头区间的唯一产出口，
   贴边切点在量化后即被丢），本轮收尾按用户拍板删掉本 PR 的 fps 派生版，不留并行判据（P1）。
