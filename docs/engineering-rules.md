...
> 这是 `CLAUDE.md` 的「按需查阅」层。`CLAUDE.md`（always 加载）= 精简核心：项目事实 + P1–P5 + D1–D6 + 规则索引 + 三闸。本文件存**触发某条规则后才查的细节**：R1–R33 详解、工作流框架、技能库映射、固化的工作纪律。
...
- **二 · 零额度给覆盖**：测试大头零花费做全——合同测试锁非法组合 + **请求构造干跑逐字段比对文档**（本地假服务器不校验格式，干跑对账才抓得住漂移）+ 免费探针（鉴权/列表端点、非 200 校验类）。**全部模式无一漏过**，含参考图/首尾帧/参考视频模式。
  - **「零额度」说的是默认走回环假供应商，不是「验收类测试一分钱都不许花」（2026-09-21 用户拍板：「改掉这个规则，规则哪里来的去哪里改，最小必要生成是允许的」）。** 走查与验收测试默认把出站钉死在回环（`NOMI_E2E_PRODUCTION_FIXTURE=1` + 只接受 127.0.0.1/localhost/::1 的 base override），此时付费额度**本来就花不出去**，所以夹具里的花费上限取一个**小的正数**而不是 0——钉 0 的代价是「带价格的付款卡 → 确认 → 账本记同一价」这条链在真机走查里永远走不通（`productionRunRuntime.ts` 的 `PRODUCTION_E2E_FIXTURE_MAX_SPEND` 即此）。
  - **真的需要真实生成时，显式声明额度**：每模型 1 次、最低档、事后如实报花了多少。默认授权，不必逐次问用户；但不许把「跑真的」当成默认，也不许用一句「跑过了」代替数字。
...
### R14.3 交付前完整差异审计与分层用模

**触发**：功能任务交付前，对最终候选版做一轮完整差异审计；用户明确要求全量审计、跨多模块修复或已发现修复引入回归时，不得只审最后一次提交。这里的“全量”是本任务相对真实 merge-base 的所有改动及受影响调用链，包含已提交、暂存、未暂存、未跟踪、删除、移动、测试、技能、规则与基线。本条只核受影响框架与链路，不自动触发 R14.2 的全仓周期复查；全仓周期审计仍按本节原触发条件；测试执行档位仍由 R22 决定，不把每次文案改动变成全库付费跑测。

**分工按判断风险，不按文件大小或扩展名**：

| 工作 | 默认执行者 | 权限和交付边界 |
|---|---|---|
| 差异清单、blob/patch hash、覆盖计数、重复/漏项、命令退出码、源码身份、已有断言去向、链接核查 | 确定性脚本优先；需要整理时用中等能力模型 | 只产事实和可追溯索引，不能凭计数判代码正确；测试/快照/规则改动不自动视为低风险 |
| 上下文充分、边界清楚的低风险初审 | 中等能力模型 | 逐处读前后代码与相关调用者，列原行为、当前行为、理由和证据；无法解释或与原行为冲突即升级，不猜测补全 |
| 并发/迟到回包、持久化/迁移、权限/付款/幂等、目标与引用绑定、原交互兼容、删旧/复用、框架边界、测试削弱和有争议发现 | 强推理模型的独立审查者；主代理亲核关键证据 | 决定不变量、归属和修复范围，核真实调用链与复现；不得将主代理方案把控和交付裁决整体外包 |
| 最终收货 | 主代理，按编排手册 §1 保持实施与验收独立 | 查覆盖、复核高风险和全部未关闭项、抽查低风险通过项、亲看真实截图；模型档位和独立性分别记录 |

模型能力分层不替代 R27 的跨池独立性；跨池不可用时可继续调查修复，但该验收项保持未完成。模型名随可用账户变化，任务书记录实际模型与职责，不把具体型号写成永久规则。中等模型不可用时先用脚本完成机械工作并记明限制，剩余语义工作由有能力的审查者承担；不冒称跨模型审查已完成。中等能力不等于把整轮验收交给较弱模型；强模型也不必逐行抄清单。

**执行闭环**：

1. **冻结与取证**：记录基线、HEAD、完整工作树身份、平台/构建；先收集全量差异再按现有 owner 分配，保留跨边界接口清单。并行审源码，Electron/性能/共享状态测试按资源冲突串行。审计不边查边随意改源码。
2. **逐处作判断**：每个差异块写出具体原行为、当前行为、`correct / incorrect / uncertain`、`introduced / existing / not-a-defect / unknown`、依据及验证边界。不能用文件模板、代码首几行或“扩展了能力且未删保护”替代判断；同一缺陷涉及多个差异块时合并为一个问题，不能虚增缺陷数量。
3. **保护原行为与测试**：单独核减号行、已有断言/快照/预算/门禁改动。断言删除必须指出等价替代位置及证据，或明确获批的行为取消；安全、权限和持久化边界不得借此削弱；“新测试绿”不能证明旧功能未回归。基线重现与新增失败分开归因，后续复跑通过不能自行证明第一次失败根因。
   新增测试也须核对原契约：区分隐藏与卸载、只读与全部禁用、请求规格与供应商实际产物，不从测试标题或当前实现反推需求。关键动作须有直接结果断言，并在其他可能造成相同结果的动作之前验证（例如先验证 blur 取消，再释放鼠标）；不能用任意状态变化代替所选参数生效。发现测试预期错误时保留红测和原契约依据，只修测试，不让生产代码迁就错误断言。
4. **独立验证**：分别记录静态审查、受控单测、真实 SDK、真实 Electron、真实供应商、候选安装包与平台。按 R13 走真实任务、检查输入可点击可写入正确对象，并亲看截图；未授权花费不能执行，已授权不重复问。Windows 或其他没跑的环境标未验证。
5. **修复后复审**：已确认问题按 P2/R21 在原共享边界修根因，补失败再通过的回归证据，不放宽断言/基线。不确定项继续调查，不能被默认通过吞掉。最终版本重新生成完整差异清单；只在 patch、依赖边界与不变量均未变化且有明确理由时复用旧审查，所有新增/变更和受牵连链路重新审。测试收据按其真实代码/构建身份使用，不能改 hash 冒充最终版本已测。
6. **交付门槛**：所有差异都有可追溯判断；已确认问题完成修复和验证，未确认或无法验证项如实阻断相关完成声明。完整报告落 `docs/audit/`，附具体证据索引；最终候选代码树仍须通过 R25 分支评审，不替代 Ponytail 或用户要求的外部审查，不授权合并。

可复用现有 [`audit-core-a-changes.mjs`](../scripts/audit-core-a-changes.mjs) 收集本任务的全量文件/差异块、patch hash 和阶段结果，`--reviews` 核查审查记录；输出必须放源码树外。跨任务复用仅使用清单或 `--reviews`，不得默认加 `--run`；测试命令仍由 R22 选择。其固定 stage 列表是本次 A＋T7 审计档，不是所有任务的默认测试预算。机械收据只证覆盖与身份，不能证语义结论正确。缺项、`uncertain`、旧 hash 不算通过；生成的截图/报告若改变工作树，原 drift 记录保留，先把证据存到树外，再仅恢复已确认的生成产物，不丢弃用户代码、不伪造整轮源码未变。
...
## R33 概念的 owner 先于目录（派工与验收）

**背景**：2026-09-22 复盘发现，「一个概念一个 owner」这条纪律在本仓只在**合并点**才起作用——并行 lane 按**目录**派工，每条 lane 在自己那片文件面里做「完整的一刀」，于是同一个概念在合并之前就各自长出了第二份实现。同一轮并行里的四个实例：

| 概念 | 分裂成了什么 |
|---|---|
| 参数准入 | 两份判据两张表：`GENERATION_PLANNING_PARAMETER_KEYS` 与 `GENERATION_PLANNING_HINTS`，各带一套拒绝形状 |
| 供应商落家 | 渲染层一份排序规则、主进程一份，两边都以为自己是那份规则 |
| 花钱回滚 | 渲染层 `spendCardRollback` 替账本做决定——补偿逻辑长在第二层，账本不知道 |
| 待决身份 | 草稿键绑的是**报价身份**而不是 `operationId`，「这一笔」于是有了两种身份 |

**为什么现有规则拦不住**：R14.1 查的是「同一语义今天有几份定义」（**已经**分裂的存量）；R1 拦的是「加新必删旧」（**同一个人**看得见第一份的情况）。本条要拦的是第三种——**两条 lane 同时动手、谁都看不见对方**，分裂在合并那一刻才现形，而那时两份都写完了、都有测试、都绿。所以判据必须前移到**派工**那一步。

**一句话**：概念的 owner 先于目录——派工切的是概念，不是文件夹。

### R33.1 任务书必须带「概念占用表」

每份实施任务书（含 chip / brief）必须带这张表；**写不出这张表就不派工、不开工**：

| 概念 | 唯一 owner（文件 : 符号） | 允许谁消费 |
|---|---|---|
| 等用户 | `electron/agentLane/laneApprovalGate.ts` | lane 工具面、渲染层只读投影 |

- 「概念」是用户 / 业务层面那件事的名字（「等用户」「出价身份」「参数准入」「供应商落家」），不是类名，更不是目录名。
- owner 必须落到**文件 + 符号**；只写到目录 = 没写。
- owner 还没定的概念照样要列，标 `pending` 并写清由哪份任务书收口——「没登记」和「登记为待定」是两件事。

### R33.2 同一时段同一概念只归一条 lane

- 主会话 / 编排者维护**全局占用表**：哪个概念此刻归哪条 lane，从派工那一刻算起。
- 跨概念的改动只能由**持有者**做，或者等它合并之后再开。
- 任何执行体接任务前先查占用表；发现自己要碰的概念已被别人持有 → **停下来协调**，不得「先写了再合并时对账」。
- 目录不冲突**不是**理由：上面四个实例里两条 lane 改的文件一个都不重叠——冲突是概念级的，`git merge` 看不见。

### R33.3 验收多一问

收货 / 评审在原有三查（编排手册 [§3 收货三查](engineering/agent-orchestration-playbook.md)）之外固定加一问：

> **这一刀有没有让任何概念多出第二个 owner？**——第二份状态、第二份规则、第二条判据，或者渲染层替主进程做决定的补偿逻辑。
...
多个入口共享同一个概念时（Agent 面板 / 画布 / 外部 MCP / 批量），只有一个 owner 还不够——还要有一条会红的判据证明它们真的同源：

> **同一输入 → 各入口的出站报文逐字节相同。**

- 这条对等测试必须做成**棘轮**：**新入口必须登记**，不登记即红。
- 目的是让分歧**在测试里报错**，而不是靠人在合并窗口逐行对账——人工对账正是上面四个实例全部漏网的那一步。
