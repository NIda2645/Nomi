...
Target: kie.ai (`REDACTED-2026-08-25`)

> ⚠️ **2026-08-25：这里原本写着一个真实可用的 kie.ai API key。**
> 它随 commit `5b73682e`（2026-05-28）进入这个 **PUBLIC** 仓库，公开暴露约三个月，
> 发现时经实测仍然有效。已就地脱敏，但**脱敏不等于止损**——公开仓库的 git 历史里
> 那个值永久存在，唯一真正的补救是去 kie.ai 后台把它作废重发。
>
> 教训（别再犯）：安全验证的记录里**不要写真凭证**，写「用哪家的 key」就够了，
> 具体值对复现毫无帮助，只会变成公开泄露。当时的安全门岗没拦住，有两个原因：
> 它没有 API key 模式，而且本文件被 `ALLOWLIST` 的 `docs/.*security.*\.md` 整份放行了
> ——白名单本是给「正文要讲微信取钥」的文档开的口子，却顺带赦免了凭证。
