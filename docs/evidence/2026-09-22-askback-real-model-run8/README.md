...
- **`ask_user` 仍然 0 领域失败**（D4 改动二连着四轮成立），**也 0 参数被拒**——
  run7 那次 `recommended` 写在题层被整次拒掉（T-QA-27）**这一轮没有复发**。
  只出现过一次，不等于问题不在；T-QA-27 照旧挂着。
...
得回到 run7 发现 ① 那条路（T-QA-27：模型问对了地方却被 schema 拒掉），而不是继续压正文。

### ② T-QA-25 的新支路**连着两轮没样本**——A4 这一轮根本没调 `delete_from_canvas`
...
1. **H1 的那一格动了但没被证明**（3/11 → 1/11，单轮）。要么再跑两轮同样的 18 句看它稳不稳，
   ...
   把力气挪到 **T-QA-27**（模型问对了地方却被 schema 整次拒掉）——那一条才直接决定用户看不看得见卡。
...
3. T-QA-25 的支路连着两轮没样本。要拿到它得让 A4 真的走到 `delete_from_canvas`——
   可以考虑在用例表里补一句更明确的删除指令，或者接受它是低频路径。
4. T-QA-24 有样本了，可以定 owner 了（第 3 节）。
