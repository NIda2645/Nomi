...
其中「关掉设置面板」连着两轮为零，所以 **T-QA-24 仍然没有样本**。
...
### ② 「把那个删了」又停了 20.6 分钟（**走查缺一条支路，第二次复现**，T-QA-25）
...
已记 **T-QA-25**，本轮**没有动产品代码，也没有动走查**（任务范围外）。
...
### ⑤ `start_model_setup` 连着两轮没被调到，T-QA-24 仍无样本

`settingsPanelClosed` 全场 0。run4 是在一次**没成立的起跑**里撞到那个死锁的，正式那一轮也没撞到。
「谁在回合结束时收掉那个应用级模态」这件事在产品侧仍然是空的（T-QA-24），但这两轮都没有新证据。
