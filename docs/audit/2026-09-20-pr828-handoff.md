...
6. **上传时门禁新结果。** `gates:contracts` 已发现 `generationRunController.ts` 为 802 行，超过 800 行上限；本次检查点如实保留，不临时扩大重构、不抬基线或压行造绿。T7 取舍文档遗漏来源节已按原有核查补齐。完整运行的其余结果及最终 Ponytail 状态见 PR 正文，不能把这次上传当全绿。
...
> 打捞分支（`integration/core-a-salvage-20260921`）已把 T7 的 chunk 本地重试退回 main 的自动整页重载，
> `tests/ux/core-a-t7-recovery.e2e.mjs` 随之删除；上面两条 `NOMI_T7_*` 命令只在 PR #828 分支上存在。
...
修前新建错误进入聊天（证据见 PR #828 分支）
修后原空白分镜编辑器（证据见 PR #828 分支）
英文冷重开（证据见 PR #828 分支）

其余 Agent 原编辑器结果（证据见 PR #828 分支）与切项目后原项目三结果（证据见 PR #828 分支）随同上传；这些是对应旅程截图，不是所有布局状态的验收结论。
