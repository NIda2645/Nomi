...
describe('creation list actual subscription owner', () => {
  it('old refresh and A-B-A read acknowledgements cannot steal the mounted project', async () => {
    const calls: { project: string; read: ReturnType<typeof deferred> }[] = []
    let timer: (() => void) | undefined
    const source = readFileSync(new URL('../../creation/storyboard/useCreationRunPlans.ts', import.meta.url), 'utf8')
      .replace(/^import .*\n/gm, '').replace(/^type ProductionRunSummary.*\n/gm, '').replace(/export /g, '')
    const context = evaluate(source + '\nfunction Probe({projectId}) { const value=useCreationRunPlans(projectId); return React.createElement("scroll",{value}); }', {
      useCallback: React.useCallback, useEffect: React.useEffect, useSyncExternalStore: React.useSyncExternalStore,
      productionRunApi: { list: (project: string) => { const read = deferred(); calls.push({ project, read }); return read.promise } },
      useWorkbenchStore: { getState: () => ({}) }, setInterval: (fn: () => void) => { timer = fn; return 1 }, clearInterval: () => { timer = undefined },
    })
    const h = harness(context.Probe), second = harness(context.Probe)
    h.render({ projectId: 'A' }); const oldA = calls[0]
    h.render({ projectId: 'B' }); const b = calls.at(-1)!
    b.read.resolve([{ runId: 'B' }]); await h.settle()
    try {
      void context.refreshCreationRunPlans('A'); await h.settle()
      expect(h.node().props.value!.runs[0]?.runId).toBe('B')
      // Two same-project reads finish in reverse order; only the latest may publish.
      timer?.(); const staleB = calls.at(-1)!
      timer?.(); const freshB = calls.at(-1)!
      expect(freshB.project).toBe('B')
      freshB.read.resolve([{ runId: 'B-fresh' }]); await h.settle()
      staleB.read.resolve([{ runId: 'B-stale' }]); await h.settle()
      expect(h.node().props.value!.runs[0]?.runId).toBe('B-fresh')
      // An older rejection must not add an error to a successfully refreshed list.
      timer?.(); const failingB = calls.at(-1)!
      timer?.(); const latestB = calls.at(-1)!
      latestB.read.resolve([{ runId: 'B' }]); await h.settle()
      failingB.read.reject(new Error('late B failure')); await h.settle()
      expect(h.node().props.value!.runs[0]?.runId).toBe('B')
      expect(h.node().props.value).toHaveProperty('error', null)
      timer?.(); const switchedAwayB = calls.at(-1)!
      second.render({ projectId: 'B' }); second.close()
      expect(timer).toBeTypeOf('function')
      expect(h.node().props.value!.runs[0]?.runId).toBe('B')
      h.render({ projectId: 'A' }); const fresh = calls.at(-1)!
      switchedAwayB.read.reject(new Error('late failure from previous project')); await h.settle()
      expect(h.node().props.value).toHaveProperty('error', null)
      fresh.read.resolve([{ runId: 'fresh-A' }]); await h.settle()
      oldA.read.resolve([{ runId: 'old-A' }]); await h.settle()
      expect(h.node().props.value!.runs[0]?.runId).toBe('fresh-A')
    } finally { h.close() }
    expect(timer).toBeUndefined()
    h.render({ projectId: null }); expect(h.node().props.value!.runs).toEqual([])
    void context.refreshCreationRunPlans('A'); await h.settle()
    expect(h.node().props.value!.runs).toEqual([])
    h.close()
    h.render({ projectId: 'B' }); calls.at(-1)!.read.resolve([{ runId: 'remounted-B' }]); await h.settle()
    expect(h.node().props.value!.runs[0]?.runId).toBe('remounted-B'); h.close()
  })
})
