...
// 提炼/缓存/墓碑全在主进程 projectMemory.ts——这里只消费。项目由调用方在动作起点签发后显式传入。
...
export async function fetchProjectMemoryFacts(projectId: string): Promise<MemoryFactView[]> {
...
export async function updateProjectMemoryFact(projectId: string, factId: string, patch: { text?: string; pinned?: boolean }): Promise<MemoryFactView[]> {
  const api = getDesktopBridge()?.memory
  if (!projectId || !api) return []
...
export async function removeProjectMemoryFact(projectId: string, factId: string): Promise<MemoryFactView[]> {
  const api = getDesktopBridge()?.memory
  if (!projectId || !api) return []
...
export async function addProjectMemoryFact(projectId: string, text: string, kind = 'preference'): Promise<MemoryFactView[]> {
  const api = getDesktopBridge()?.memory
