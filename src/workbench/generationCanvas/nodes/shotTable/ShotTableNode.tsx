...
import { canRestartDeconstruction } from './deconstructionLifecycle'
...
function ShotTableContent({ node: rawNode, selected, readOnly = false, flowDensity }: NodeProps & { flowDensity?: 'full' | 'compact' | 'card' }): JSX.Element {
  ...
  const interrupted = facts?.source.status === 'interrupted' || facts?.source.status === 'cancelled'
  ...
  return <article data-node-id={node.id} data-kind="shot_table" data-testid="shot-table-node" data-density={density}
    ...
    {density !== 'card' && facts?.source.errorMessage && rows.length > 0 && !interrupted
    ...
    {density !== 'card' && interrupted && facts?.source.errorMessage
      && <div data-testid="shot-table-interrupted-notice" className="shrink-0 border-b border-nomi-line-soft px-3 py-1 text-micro text-nomi-ink-60">{facts.source.errorMessage}</div>}
    ...
    {density !== 'card' && <footer className="nodrag generation-canvas-react-flow__no-pan flex h-10 shrink-0 items-center gap-2 border-t border-nomi-line-soft px-3" onPointerDown={event => event.stopPropagation()} onClick={event => event.stopPropagation()} onDoubleClick={event => event.stopPropagation()}>
      ...
      {facts && canRestartDeconstruction(facts.source.status) && facts.source.status !== 'idle' && !readOnly && <WorkbenchButton size="sm" variant={facts.source.status === 'interrupted' ? 'primary' : 'default'} data-testid="shot-table-restart-deconstruction" disabled={!sourceAvailable} title={!sourceAvailable ? t('shotTable.sourceVideoMissing') : undefined} onClick={() => { withProjectAction(project => { void deconstructToShotTable(facts.source.sourceNodeId, project).catch(cause => setError(cause instanceof Error ? cause.message : String(cause))) }) }}>{t('shotTable.retry')}</WorkbenchButton>}
      {facts?.source.status === 'idle' && !readOnly && <WorkbenchButton size="sm" variant="default" disabled={!sourceAvailable} title={!sourceAvailable ? t('shotTable.sourceVideoMissing') : undefined} onClick={() => { withProjectAction(project => { void deconstructToShotTable(facts.source.sourceNodeId, project).catch(cause => setError(cause instanceof Error ? cause.message : String(cause))) }) }}>{t('shotTable.retry')}</WorkbenchButton>}
