...
function ShotTableContent({ node: rawNode, selected, readOnly = false, flowDensity }: NodeProps & { flowDensity?: 'full' | 'compact' | 'card' }): JSX.Element {
  ...
  // production 表的行从 Run 落地节点 derive；占位三态读落地 store 里那份 Run（只读投影缓存，非第二真相）。
  const landedRun = useProductionCanvasLandingStore(state => (table?.source.kind === 'production' && state.run?.runId === table.source.runId ? state.run : null))
  const rows = React.useMemo(() => table ? selectShotTableRows({ table, designs, nodes, imageModelOptions, videoModelOptions, run: landedRun }) : [], [table, designs, nodes, imageModelOptions, videoModelOptions, landedRun])
