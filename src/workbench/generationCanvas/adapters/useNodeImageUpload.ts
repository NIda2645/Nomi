...
export function useNodeImageUpload(nodeId: string, source: string): (dataUrl: string, file: File, context?: ProjectExecutionContext) => void {
  ...
  return React.useCallback(
    (dataUrl: string, file: File, originatingContext?: ProjectExecutionContext) => {
      let context: ProjectExecutionContext
      try { context = originatingContext ?? captureCurrentProjectExecutionContext(); context.assertCurrent() } catch { return }
