...
export function scriptDraftContentHash(content: string): string {
  // This module is imported by the renderer. Keep the hash implementation
  // synchronous and browser-safe instead of pulling Node's `crypto` into the
  // browser bundle. TextEncoder gives us the same UTF-8 bytes as
  // `createHash('sha256').update(content, 'utf8')`.
