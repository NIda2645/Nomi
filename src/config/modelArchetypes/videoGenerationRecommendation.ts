...
import type { ArchetypeMode, ArchetypeReferenceSlot, ModelArchetype } from "./types";
...
const scoreMode = (mode: ArchetypeMode, summary: ReferenceSummary, input: VideoGenerationRecommendationInput): number => {
  ...
  if (typeof input.goals?.durationSeconds === "number") {
    score += durationSupport(mode, input.goals.durationSeconds)?.supported ? 30 : -30;
  }
  return score;
};
...
export function recommendVideoGeneration(
