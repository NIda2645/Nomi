import type { ModelParameterControl, ModelParameterControlOption } from '../modelCatalogMeta'
import type {
  ArchetypeIntent,
  ArchetypeMode,
...
/**
 * User-authored, provider-independent capability declaration stored at
 * `model.meta.customCapabilityContract`. It deliberately excludes transport mappings,
 * authentication and adapter evidence: those describe how to call a provider, not what
 * inputs and modes the model accepts.
 */
...
function parseMode(value: unknown, modelKind: ModelArchetype['kind']): CustomCapabilityModeV1 | null {
  ...
  // A combined output may deliberately reuse one source key: the builder deletes all source
  // keys before writing the combined value. Fixed params and user controls remain live, so
  // collisions with those are ambiguous and invalidate the mode.
  const outputKeys = [
...
function normalizeContractUnsafe(value: unknown): CustomCapabilityContractV1 | null {
  ...
  const kind = KINDS.has(value.kind as ModelArchetype['kind']) ? value.kind as ModelArchetype['kind'] : null
