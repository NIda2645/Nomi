export const zhModelSetup = {
  addModel: '添加一个 AI 模型',
  intro: '填中转地址，按需填写 Key，拉取它开放的模型；图片 / 视频 / 文本一次接入，类型自动判好可改。',
  ...
  fetchHintNoApiKey: '填好接入地址，会自动拉取这个上游开放的模型',
  fetchModels: '拉取模型',
  ...
  connectedReachabilityOnlyNoApiKey: '地址可以访问 · 你选的都是图片 / 视频模型，不走「接口协议」，能不能出片要真跑一次才知道',
...
export const enModelSetup = {
  ...
  intro:
    'Enter a gateway URL and, when required, a key to fetch its models. Text, image, and video models are classified automatically and can be adjusted.',
  ...
  fetchHintNoApiKey: 'After you enter the Base URL, Nomi will fetch the models available from this upstream',
