...
import { admitMediaImport, type MediaImportRejection, type MediaImportSurfaceId } from "../shared/contracts/mediaImportPolicy";
import { readStorageCapacity } from "./storageCapacity";

import { parseLocalAssetUrl } from "../protocol/localProtocol";
import { ensurePlayableVideoBytes, playableMp4FileName, transcodeFileToPlayableMp4IfNeeded } from "./videoImportNormalize";
...
async function importNativeSourcePath(
  ...
  if (!effectiveContentType.startsWith("video/")) {
    return copyAssetFile(projectId, sourcePath, storedName, effectiveContentType, baseMeta, feedback);
  }
  ...
  try {
    ...
    return await copyAssetFile(projectId, sourcePath, storedName, effectiveContentType, baseMeta, feedback);
  } finally {
