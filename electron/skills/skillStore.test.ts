...
describe("isCraftSkill（对外暴露的安全边界）", () => {
  ...
  it("keeps the built-in craft + whitelisted skills exposed (迁移不回归)", () => {
    ...
    // 显式白名单的运营技能同样对外（main 侧既有行为，不回退）
    expect(isCraftSkill(at("model-integration", "builtin"))).toBe(true);
  });
  ...
  it("never exposes a user-imported skill, whatever it is named", () => {
    ...
    // 即便撞上白名单目录名，用户导入的也不该对外
    expect(isCraftSkill(at("model-integration", "user"))).toBe(false);
  });
