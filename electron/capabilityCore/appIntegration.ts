...
import {
  createCatalogModelPricingResolver,
  createCatalogShotPriceResolver,
} from "../productionRun/catalogPricingResolver";
...
import { createDefaultAuthorities } from "./integrationAuthorities";
...
let reworkProductionShotHook:
  | ((input: { projectId: string; runId: string; shotId?: string }) => Promise<ProductionActionResult>)
  | null = null;
let resumeProductionBatchHook:
  | ((input: { projectId: string; runId: string; reason: "budget" | "manual" }) => Promise<ProductionActionResult>)
  | null = null;
 ...
 */
export async function reworkProductionShot(input: {
  projectId: string;
  runId: string;
  shotId?: string;
...
export async function resumeProductionBatch(input: {
  projectId: string;
  runId: string;
...
export async function startCapabilityCore(
  ...
  try {
    ...
    const generationRegistry =
      ...
      createCatalogModuleRegistry(undefined, { readinessByProvider: providerBootstrap.readinessByProvider });
      ...
      catalog.models
        .filter(
          (model) =>
            model.enabled &&
            model.kind === "video" &&
            modelHasPublishedExecution(model, { mappings: catalog.mappings }),
        )
        .map((model) => ({
    ...
    const resolveModelPricing = (providerId: string, modelId: string) =>
      createCatalogModelPricingResolver(readCatalog().models)(providerId, modelId);
    const resolveShotPrice = (contract: Parameters<ReturnType<typeof createCatalogShotPriceResolver>>[0]) =>
      createCatalogShotPriceResolver(readCatalog().models)(contract);
      ...
      return landCanvasForRun(run, {
        ...
        bindShotNodes: async (boundProjectId, boundRunId, expectedRevision, bindings) => {
          await generationService.command(boundProjectId, boundRunId, {
            commandId:
              `canvas-landing:${boundRunId}:bind:${bindings.map((binding) => `${binding.shotId}=${binding.nodeId}`).join(",")}`.slice(
                0,
                200,
              ),
            expectedRevision,
    ...
    const pushShotResultToRenderer = async (projectId: string, runId: string, shotId: string): Promise<void> => {
      ...
      const job = run.jobs.find(
        (candidate) =>
          typeof candidate.metadata?.shotId === "string" &&
          candidate.metadata.shotId === shotId &&
      ...
      try {
        ...
        console.warn(
          "[nomi:production] push shot result failed:",
    ...
    const buildSubmissionForRun = (run: {
      projectId: string;
      generationPlan?: { candidate: { providerId: string } };
      jobs: Array<{ provider: string }>;
    }) => {
      const providerId = run.generationPlan?.candidate.providerId ?? run.jobs[0]?.provider;
      const provider = providerId
        ? providerBootstrap.providers.find((candidate) => candidate.providerId === providerId)
        : undefined;
      ...
      return createProductionGenerationSubmission({
        ...
        provider,
        resolveShotPrice,
        materializeOutput: ({ projectId, providerTaskId, output }) =>
          outputMaterializer.materialize({ projectId, providerTaskId, output }),
      });
    };
    ...
    const driveScheduler = (
      projectId: string,
      runId: string,
      scheduler: MultiShotBatchScheduler,
      label: string,
    ): void => {
      const key = `${projectId}:${runId}`;
    ...
    const generationPlanning =
      authorities.generationPlanning ??
      createGenerationPlanningHandler({
        ...
        providerReadiness: ({ providerId }) =>
          providerBootstrap.readinessByProvider[providerId] ?? {
            providerReady: false,
            missingForSubmit: ["configured_provider"],
          },
        start: async (operation, lease) => {
          const provider = providerBootstrap.providers.find(
            (candidate) => candidate.providerId === operation.contract?.providerId,
          );
          const projectRoot = resolveWorkspaceProjectDir(lease.projectId, getWorkspaceRepositoryDeps());
          if (!provider || !projectRoot || !operation.contract)
            return {
        ...
        reconcile: async (operation, outcome, lease) => {
          if (outcome === "not_found")
            return { operationId: operation.operationId, outcome, nextAction: "manual_review" };
          const provider = providerBootstrap.providers.find(
            (candidate) => candidate.providerId === operation.contract?.providerId,
          );
          const projectRoot = resolveWorkspaceProjectDir(lease.projectId, getWorkspaceRepositoryDeps());
          if (!provider || !projectRoot || !operation.contract)
            return { operationId: operation.operationId, outcome, nextAction: "manual_review" };
          if (!provider.query || !provider.capabilities.query)
            return {
              operationId: operation.operationId,
              outcome,
              nextAction: "manual_review",
              recoveryNotice: "该供应商没有可用的任务查询；请到供应商核对。",
            };
          const submission = createProductionGenerationSubmission({
            repository: generationService.repository,
            projectRoot,
          ...
          try {
            ...
            if (code === "provider_materialization_unsupported" || code === "materialization_failed")
              return {
                operationId: operation.operationId,
                outcome,
                nextAction: "manual_review",
                recoveryNotice: "供应商任务已完成，但结果还没有安全落到 Nomi 项目；请到供应商核对或稍后重试。",
              };
            throw error;
          }
        },
    ...
    reconcileOpenProjectHook = (projectId: string) => {
      void (async () => {
        try {
          const summaries =
            typeof generationService.repository.list === "function" ? generationService.repository.list(projectId) : [];
          ...
          console.warn(
            "[nomi:production] open-project canvas reconcile failed:",
            error instanceof Error ? error.message : String(error),
          );
        }
        ...
        try {
          ...
          console.warn(
            "[nomi:production] resume unfinished runs failed:",
            error instanceof Error ? error.message : String(error),
          );
        }
      })();
    ...
    const reworkProductionShot = async (input: {
      projectId: string;
      runId: string;
      shotId?: string;
    }): Promise<ProductionActionResult> => {
      ...
      if (!run || !run.generationPlan?.shots || run.generationPlan.shots.length === 0)
        return { ok: false, code: "not_multishot" };
      const submission = buildSubmissionForRun(run);
      ...
      const shot = shotId
        ? (fresh?.generationPlan?.shots ?? []).find((candidate) => candidate.shotId === shotId)
        : undefined;
      ...
      const shotSummary =
        typeof shot?.candidate.prompt === "string" && shot.candidate.prompt.trim()
      ...
      try {
        confirmation = (await confirm({ challengeToken: challenge.token })) as {
          confirmed?: unknown;
          receiptId?: unknown;
        } | null;
      } catch (error) {
      ...
      const receiptId =
        confirmation?.confirmed === true && typeof confirmation.receiptId === "string" ? confirmation.receiptId : "";
      if (!receiptId) {
      ...
      try {
        ...
        const approvalHash = approving.generationPlan.shots
          ? approving.generationPlan.planHash
          : approving.generationPlan.contract?.contractHash;
      ...
      if (kicking) {
        const scheduler = buildSchedulerForRun(
          projectId,
          runId,
          kicking,
          maximumCost > 0 ? { raisePlanAuthorizationTo: kicking.budget.authorized + maximumCost } : undefined,
        );
        if (scheduler) {
    ...
    const resumeProductionBatch = async (input: {
      projectId: string;
      runId: string;
      reason: "budget" | "manual";
    }): Promise<ProductionActionResult> => {
      ...
      const scheduler = buildSchedulerForRun(
        ...
        runId,
        running,
        raiseTo !== undefined ? { raisePlanAuthorizationTo: raiseTo } : undefined,
      );
    ...
    const integrationSessions = installRuntimeIntegrationSessionService(
      createRuntimeIntegrationSessionService({
        approvalReceiptAuthority: authorities.approvalReceiptAuthority || defaults.approvalReceiptAuthority,
        runTask,
        fetchTaskResult,
