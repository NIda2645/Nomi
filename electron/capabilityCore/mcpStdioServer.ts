...
let runtimeIntegrationSessions: IntegrationSessionService | null = null;
...
async function callViaRpc(
  ...
  try {
    ...
    if (options?.signal?.aborted)
      throw options.signal.reason instanceof Error ? options.signal.reason : new Error("MCP request cancelled");
...
async function invoke(
  ...
  return dispatchAndEnrich(method, params, {
    ...
    productionRuns,
    integrationSessions:
      authorities.integrationSessions ||
      runtimeIntegrationSessions ||
      (runtimeIntegrationSessions = createRuntimeIntegrationSessionService({
        approvalReceiptAuthority: authorities.approvalReceiptAuthority,
        runTask,
        fetchTaskResult,
...
export async function startMcpStdioServer(authorities: McpStdioServerOptions = {}): Promise<void> {
  ...
  const previewServer = await startArtifactPreviewHttpServer(
    withAssetPreview(productionRuns, (projectId) =>
  ...
  const videoModelCandidates = buildVideoModelCandidates(
    catalog.models
      .filter(
        (model) =>
          model.enabled && model.kind === "video" && modelHasPublishedExecution(model, { mappings: catalog.mappings }),
      )
      .map((model) => ({
  ...
  const protocol = createMcpProtocol({
    ...
    invoke: (method, params, options) =>
      invoke(method, params, options, { ...authorities, generationPlanning, generationPolicy }),
    ...
    confirmGenerationInNomi: async (challenge) => {
      const challengeToken =
        challenge.handoff && typeof challenge.handoff.challengeToken === "string"
