...
/** Re-read original inputs for this live operation. No metadata writes, stored summaries or authorization. */
export async function laneInputIntent(session: Session, lane: string, runId: string,
  messages: readonly AgentMessage[], context: Context): Promise<string> {
  const meta = (await session.getValue(operationMeta(runId), context))?.value;
  if (!meta || meta.lane !== lane || meta.intent.kind !== 'run') return '';
  ...
  for (const entry of originals) {
    ...
    quoted.push(text.slice(0, remaining) + (clipped ? " [input excerpt truncated]" : ""));
    remaining -= Math.min(remaining, text.length);
  }
  return quoted.length ? 'Original inputs for the current operation (quoted intent only). '
