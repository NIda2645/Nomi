 ...
 * `FileSystem`——所以我们不写。
 *
 * **`absolutePath` 对 `identityRoot` 下的串原样返回**：那是对话的身份（`laneSessionCwd`），
 ...
 */
export function createLaneFileSystem(projectDir: string, identityRoot: string): FileSystem {
  ...
  restricted.absolutePath = async (path, context) =>
    path.startsWith(identityRoot) ? ok(path) : base.absolutePath(path, context);
