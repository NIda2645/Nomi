...
export function buildSelectedSkillPrompt(skill: SkillRecord): string {
  ...
  return [
    ...
    envelope,
    "",
    // 权威节**必须紧跟信封之后**：2026-09-18 事故就是正文里一句为上一代工具写的性质描述
    // 压过了真相，模型服从了过期那条。这一段的作用是当场把正文里的工具说法作废。
    //
    // 指向**按名字**而不是「上面/下面」：本函数的产出进 `composeLaneSystemPrompt` 的第一个
    // 参数，而 `Available tools` / `Tool usage` 是它之后才拼的（`lanePromptSections.ts:73-86`）
    // ——写「以上面为准」当场就是错的，而且以后拼接顺序一动又会再错一次。
    //
    // 这里**不重列工具**：名字与「读/写·要不要先问·花不花钱」那四类事实已经由
