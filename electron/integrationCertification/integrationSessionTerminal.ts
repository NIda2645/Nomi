...
/**
 * 看门狗把会话收成终态之后那一次落盘，**不许拖垮进程**。
 *
 * 它在两个地方跑，两处抛出去都没人接：启动期的 `resumeInterrupted()` 一路走到 `main.ts`
 * 的 `.catch` → `app.quit()`（和「接入会话超过 100 条」那次静默退出是同一种死法，
 * 2026-09-21 验收方用 >1MiB 的盘实测复现过）；周期定时器里则是一个未捕获异常。
 *
