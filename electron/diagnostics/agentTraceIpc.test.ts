...
const mocks = vi.hoisted(() => ({ handle: vi.fn(), guard: vi.fn(), revealFile: vi.fn(),
  activeProject: vi.fn(), projectDirectory: vi.fn(), nativeRequireBase: undefined as string | undefined }));
vi.mock('electron', () => ({ ipcMain: { handle: mocks.handle }, shell: { showItemInFolder: mocks.revealFile } }));
...
it('resolves the default native loader from the emitted diagnostics directory', async () => {
  ...
  try {
    ...
    await mkdir(path.join(projectDir, 'main.trace'), { recursive: true });
    await writeFile(path.join(projectDir, 'main.trace/trace.md'), '# Trace');
    mocks.projectDirectory.mockReturnValue(projectDir);
    mocks.revealFile.mockResolvedValue('');
    registerAgentTraceIpc();
    expect(await mocks.handle.mock.calls[0][1]({}, 'main')).toEqual({ ok: true });
    expect(mocks.revealFile).toHaveBeenCalledWith(path.join(projectDir, 'main.trace/trace.md'));
...
it('rejects untrusted senders before resolving any project or revealing files', async () => {
  ...
  expect(deps.revealFile).not.toHaveBeenCalled();
});
...
it('opens the native-derived current project directory, or the requested validated lane', async () => {
  ...
  expect(deps.revealFile).toHaveBeenLastCalledWith('/isolated/project/.nomi/agent-sessions/session.trace/index.md');
  expect(await invoke({}, '新对话 2')).toEqual({ ok: true });
  expect(deps.traceDirectory).toHaveBeenLastCalledWith('/isolated/project', '新对话 2');
  expect(deps.revealFile).toHaveBeenLastCalledWith('/isolated/project/.nomi/agent-sessions/session.trace/trace.md');
});
...
it('returns bounded failures for shell errors and rejected native reads', async () => {
  const { deps, invoke } = setup();
  deps.revealFile.mockImplementation(() => { throw new Error('OS error with private path'); });
  expect(await invoke({})).toEqual({ ok: false, reason: 'open-failed' });
  deps.traceDirectory.mockRejectedValue(new Error('private provider information'));
  expect(await invoke({})).toEqual({ ok: false, reason: 'open-failed' });
  expect(deps.revealFile).toHaveBeenCalledTimes(1);
});

it('does not ask the file manager to reveal a missing or non-file derived trace', async () => {
  const { deps, invoke } = setup();
  deps.isTraceFile.mockResolvedValue(false);
  expect(await invoke({}, 'main')).toEqual({ ok: false, reason: 'open-failed' });
  expect(deps.revealFile).not.toHaveBeenCalled();
});
