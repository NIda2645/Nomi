...
describe("desktop credential startup ordering", () => {
  ...
  it("repairs broken launchers at startup without waiting for a renderer notice", () => {
    const integration = fs.readFileSync(new URL("./capabilityCore/appIntegration.ts", import.meta.url), "utf8");
    expect(integration).toContain("repairStaleMcpConfigs()");
    expect(integration).not.toContain("notifyHostConfigRepaired");
  });
});
