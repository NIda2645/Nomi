 ...
 *
 * 显式指定优先：`{vendorKey,modelKey}` 就按它解；`"cloud"` 与缺省都走自动解析。
 * **指定了却解不出来就返回 null**，不悄悄回落到别家——「我选了本地，它却偷偷走了云端
 * 并且扣了钱」是这条路上最糟的失败。
 *
 * 自动那一半 2026-09-17 修过：原来写的是 `findExecutableModel(brain.vendor, "", "audio")`——
