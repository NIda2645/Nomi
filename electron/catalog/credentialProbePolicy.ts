...
//
// 拍板的修法是**免费探测**，两句话：
//   ① 能用免费端点就用（种子声明的零费用探测 / OpenAI 兼容的 `GET /models`）；
...
// ── 缺省是 fail-closed ──
//
// 种子的 `livenessProbe` 不声明 `cost` 就判 `paid`（= 先问）。理由是 T-MO-20 的真实教训：
...
export type CredentialProbePlan =
   ...
   */
  | { kind: 'seed-probe'; cost: CredentialProbeCost; probe: NonNullable<VendorSeed['livenessProbe']> }
  /**
...
export function credentialProbePlan(vendorKey: string): CredentialProbePlan {
  ...
  if (seed.livenessProbe) {
    ...
    return { kind: 'seed-probe', cost: seed.livenessProbe.cost ?? 'paid', probe: seed.livenessProbe }
  }
