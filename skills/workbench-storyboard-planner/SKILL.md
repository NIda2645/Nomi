...
- **图片分镜（`shotKind: "image"`，默认）**：每镜是**一张静态画面**（图生图）。
  - `durationSec` **省掉不填**（静帧没有时长；工具只收正数，填 `0` 会被当场拒）；
...
- `durationSec`：时长（秒），**仅视频分镜**——**别拍脑袋、别信剧本「约 Ns」标注**（画面骨架估时系统性低估对白与表演），按下方 **§演时换算法** 给每镜算出真实表演秒数填入。落画布时系统会钳到所选模型上限；**算出超过单条上限的拍子，拆成连续多镜**（见 §演时换算），别硬塞进一镜被截断。**图片分镜省掉这个字段，不要填 `0`。**
