...
try {
  ...
  await historyVideo.waitFor({ state: 'attached', timeout: 10_000 })
  check('悬停历史视频真实播放元素可见', await historyVideo.isVisible())
  ...
  check('展开后素材库面板可见', await sidebar.locator('section[aria-label="素材库"]').isVisible())
  await backToLibrary()
