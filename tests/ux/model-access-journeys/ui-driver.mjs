...
export async function launchJourneyUi({ journey, recorder }) {
  ...
  async function expandGenerationProviders() {
    // Model settings now opens straight into the "add services" home
    // (ModelSettingsHome): the relay/custom-API entry is a top-level row, not a
  ...
  async function openRelayWizard() {
    ...
    await win.getByRole('button', { name: /自定义 API \/ 中转站/ }).first().click()
    await win.getByText('添加一个 AI 模型', { exact: true }).waitFor({ state: 'visible', timeout: 8000 })
  }
