...
function requiredPhases(journeyId) {
  ...
  if (['J05', 'J07', 'J12'].includes(journeyId)) phases.push('recovered')
  return phases
}
...
export async function runJourneyFile(fileName) {
  // The journey fact source (manifest.mjs) stores `script` as a repo-relative
  // path; each .walk.mjs entrypoint passes its own bare basename. Match on
  // basename so the single manifest owns the script→journey mapping.
  const wanted = path.basename(fileName)
  const owned = MODEL_ACCESS_JOURNEYS.filter((journey) => path.basename(journey.script) === wanted)
