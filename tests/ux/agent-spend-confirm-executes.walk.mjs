...
try {
  ...
  // 改后的**清晰度**到没到线缆：这条分支上记录、不断言——文件头 ③ 写明了它今天不成立
  //（`compileParameters` 把档案外的参数丢掉），修复在步骤 A 那条 lane 上、尚未并进本分支。
  // 记下实测值：并线之后把这一行换成 `expect(...).toContain(EDITED_RESOLUTION)` 即可。
  walk.report.wireCarriesEditedResolution = JSON.stringify(submitted).toLowerCase().includes(EDITED_RESOLUTION.toLowerCase())
