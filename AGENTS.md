...
> - **L2 触发才查** = `docs/engineering-rules.md`：R1–R17 详解 + 工作流框架 + 技能库映射 + 固化纪律。规则索引指明每条住哪，触发某条才去读它。（`docs/coding-standards.md` = 通用编码规范补充。）
>
...
| R16 | 真实用户任务测试 | 功能交付必须固化并跑通主路径与降级路径的真实用户任务闭环 |
| R17 | 解决状态必须可交付 | 侧分支只能称“已实现”；验证通过且提交已进入远端目标分支后才能称“已解决” |
