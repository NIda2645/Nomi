...
| R21 | React Flow 生成画布单内核 | 生产生成画布只允许 `@xyflow/react`：`GenerationCanvas` 是唯一稳定入口，禁第二 renderer、engine flag、fallback、并行实现；Zustand/domain/project snapshot 是唯一业务与持久化真相源，React Flow state 只做渲染投影；连接必须走现有 graph actions；交互变化须补结构/adapter 测试与真实 Electron 走查。onboarding 静态只读工作流图不属此 renderer。|
