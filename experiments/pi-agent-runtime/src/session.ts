...
import { createNomiModelRuntime, type NomiModelConfig } from './model.js';
import { createNomiResourceLoader } from './resources.js';
import { createHostTools, type HostToolDefinition } from './tools.js';
...
/** R0 experiment only: the SDK owns the loop; Nomi owns every input and tool effect. */
export async function createControlledSession(options: ControlledSessionOptions) {
  ...
  session.agent.streamFunction = (chosen, context, streamOptions) => {
    ...
    return sdkStream(chosen, context.systemPrompt === sdkBasePrompt
      ? { ...context, systemPrompt: options.systemPrompt } : context, streamOptions);
  };
