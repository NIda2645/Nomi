 ...
 *
 * **2026-09-18 投影化之后，这两端在 17 个动词上已经是同一份 schema** 了（模型面 = 宿主面
 * `.omit(宿主自补的字段)`，`verbs/verbProjections.ts`）。对那些动词，下面 R1–R3 是按构造成立的——
 * 它们照跑，但跑出来的是恒真，成本是几毫秒。**这道门今天真正在管的是剩下那三类**：
 ...
 * 裁决 §3 的判断「投影之后 R1–R3 退化成一句子集断言」只对**纯投影**的那 17 个成立；上面那三类里
 * 缝还在，而 R1/R2/R2b/R3 正是量那条缝的唯一一把尺子（2026-09-18 的 A/B/C/D 四类全出在那三类里）。
 * 恒真地跑 17 个动词的成本是几毫秒，删掉的成本是那三类重新变成盲区——所以留着。
