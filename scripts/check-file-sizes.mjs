...
const ALLOWLIST = {
  ...
  "electron/integrationCertification/integrationSession.ts": 1338, // 1695→1652（2026-09-10 把 workflow 绑定净化与「谁来编译说明卡」的裁决各抽成独立模块）
  ...
  "src/workbench/generationCanvas/nodes/BaseGenerationNode.tsx": 690, // …→ 731（2026-08-25 P4 S6：多镜叠加合一 ProductionShotOverlays）→ 713（2026-08-29 React Flow 单内核：移除旧布局与缩放分支）→ 710（2026-09-07 agent-artifact：正文/浮条插槽移出壳 + composer 排除表收进 resolveRenderKind）→ 695（2026-09-14 媒体预览：尺寸 reconciler 收成一处、注释精简）→ 690（2026-09-15 并 main：#763 的 S3 把四个整表选择器收进 canvasNodeGenerationIndex，两次瘦身叠加）
